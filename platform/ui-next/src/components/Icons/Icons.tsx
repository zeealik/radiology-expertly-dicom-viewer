...
// Generic UI glyphs come from Lucide through a single adapter so the whole set
// shares one 1.5px stroke. Each binding keeps the icon's original OHIF name, so
// every registry key and kebab-case alias below resolves unchanged.
// Domain glyphs — measurement tools, layout grids, window-level presets — stay
// hand-drawn; Lucide has no equivalent for them.
import {
...
import { fromLucide } from './Sources/lucideAdapter';

const Add = fromLucide(LucidePlus);
const Alert = fromLucide(LucideAlertTriangle);
const AlertOutline = fromLucide(LucideAlertTriangle);
const ArrowLeftBold = fromLucide(LucideArrowLeft);
const ArrowRight = fromLucide(LucideArrowRight);
const Cancel = fromLucide(LucideX);
const Checked = fromLucide(LucideCheck);
const ChevronClosed = fromLucide(LucideChevronRight);
const ChevronLeft = fromLucide(LucideChevronLeft);
const ChevronOpen = fromLucide(LucideChevronDown);
const Clear = fromLucide(LucideX);
const Clipboard = fromLucide(LucideClipboard);
const Close = fromLucide(LucideX);
const Code = fromLucide(LucideCode);
const Copy = fromLucide(LucideCopy);
const Database = fromLucide(LucideDatabase);
const Delete = fromLucide(LucideTrash2);
const Download = fromLucide(LucideDownload);
const Export = fromLucide(LucideExternalLink);
const EyeHidden = fromLucide(LucideEyeOff);
const EyeVisible = fromLucide(LucideEye);
const GearSettings = fromLucide(LucideSettings);
const Hide = fromLucide(LucideEyeOff);
const Info = fromLucide(LucideInfo);
const JumpToSlice = fromLucide(LucideMoveRight);
const ListView = fromLucide(LucideList);
const LoadingSpinner = fromLucide(LucideLoader2);
const Lock = fromLucide(LucideLock);
const Minus = fromLucide(LucideMinus);
const More = fromLucide(LucideMoreHorizontal);
const MultiplePatients = fromLucide(LucideUsers);
const NavigationPanelReveal = fromLucide(LucidePanelLeftOpen);
const Patient = fromLucide(LucideUser);
const Pin = fromLucide(LucidePin);
const Plus = fromLucide(LucidePlus);
const PowerOff = fromLucide(LucideLogOut);
const Redo = fromLucide(LucideRedo2);
const Refresh = fromLucide(LucideRotateCw);
const Rename = fromLucide(LucidePencil);
const Search = fromLucide(LucideSearch);
const Series = fromLucide(LucideLayers);
const Settings = fromLucide(LucideSettings);
const Show = fromLucide(LucideEye);
const SidePanelCloseLeft = fromLucide(LucidePanelLeftClose);
const SidePanelCloseRight = fromLucide(LucidePanelRightClose);
const ThumbnailView = fromLucide(LucideLayoutGrid);
const Undo = fromLucide(LucideUndo2);
//
//
